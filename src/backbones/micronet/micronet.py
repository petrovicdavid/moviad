...
class MicroNet(nn.Module):
    def __init__(self, cfg, input_size=224, num_classes=1000, teacher=False):
        ...
        from .microconfig import get_micronet_config
        ...
        self.num_features = input_channel
...
class Activation():
    MODULE = 'DYShiftMax'
    ACT_MAX = 2.0
    LAST_SE_OUP = False #use se-oup for the last 1x1 conv
    LINEARSE_BIAS = False
...
class MicroNetBB(MicroNet):
    def __init__(self, *args, **kwargs):
        super(MicroNetBB, self).__init__(*args, **kwargs)
...
def micronet_new_fc(hparams, device):
    modules = torch.nn.ModuleDict({})
    cfg = Cfg()
    state_dict = prepare_dictionary(hparams.micronet_default_path, hparams)
    backbone = micronetBB(hparams, device, state_dict=state_dict)
    if hparams.linear_finetune:
        backbone.features.requires_grad_(False)
        backbone.avgpool.requires_grad_(False)
    modules["backbone"] = backbone

    head = micronetH(hparams, device, backbone.num_features, state_dict)
    modules["head"] = head

    if not ("backbone" in state_dict or "head" in state_dict):
        ic(backbone.load_state_dict(state_dict, strict=False))
    return modules

#def micronetBB(hparams, device, state_dict=None):
def micronetBB(device, state_dict=None):
    #input_size=hparams.input_shape[-1]
    backbone = MicroNetBB(cfg=Cfg(), input_size = 224, num_classes=1000).to(device)
    backbone.eventually_load(state_dict, "backbone")
    return backbone.to(device)

def micronetH(hparams, device, input_features, state_dict=None):
    head = nn.Sequential(
        SwishLinear(input_features, hparams.head_size),
        nn.Dropout(0.05),
        SwishLinear(hparams.head_size, hparams.num_classes )
    )
    head.eventually_load(state_dict, "head")
